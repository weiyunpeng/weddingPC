...
<template>
    <div class="vue-waterfall-easy" :style="isMobile? '':{width:boxWidth*columnCount+'px',left:'50%',marginLeft: -1*boxWidth*columnCount/2 +'px'}">
        <ul class="img-box" v-for="(v,i) in imgsArrC" :key="i" :style="{padding:gap/2+'px',width: isMobile ? '' : boxWidth+'px'}">
            <li class="img-inner-box">
    ...
    methods: {
        ...
        initColsHeightArr() { // 第一行元素的高度组成的数组-初始化
            ...
            for (var i = 0; i < this.columnCount; i++) {
                this.imgBoxEls[i].style.position = 'absolute' // 重置下position
